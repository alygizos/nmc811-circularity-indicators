...
def load_material_parameters(csv_file='pci_material_parameters.csv'):
    """
...
def plot_pci_with_error_bars(csv_file='pci_material_parameters.csv', n_samples=1000,
    ...
    colors = [
    "#b2182b",  # strong warm red
    "#d6604d",  # mid warm coral
    "#f4a582",  # soft salmon
    "#fddbc7",  # light warm-neutral
    "#d1e5f0",  # light cool-neutral
    "#92c5de",  # sky blue
    "#4393c3",  # medium blue
    "#2166ac"   # deep blue
    ]
    ...
    sep_x = x_pos[-2] + gap/2      # position between last material and Total PCI
    ax.axvline(
    sep_x,
    color='black',
    linestyle=(0, (4, 4)),
    ...
    ax.set_ylabel('PCI', fontsize=23, fontweight='bold')
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(label_names, ha='center', fontweight='bold', fontsize=23)
...
if __name__ == '__main__':
    ...
    print("="*70)
    final_pci, material_pcis = print_pci_results()
    ...
    print("  - Samples: 1000")
    print("  - Confidence: 95%")
    fig, ax, stats = plot_pci_with_error_bars(n_samples=1000, uncertainty_pct=0.05)
    ...
    materials, _ = load_material_parameters()
    for material in materials.keys():
        save_path = f'pci_tornado_{material.replace(" ", "_")}.png'
        impacts, sorted_params = tornado_plot(material_name=material, variation_pct=0.05,
                                              save_path=save_path)
        print("\n")
    ...
    for material in materials.keys():
        save_path = f'pci_sobol_{material.replace(" ", "_")}.png'
        Si = sensitivity_analysis(material_name=material, n_samples=512, save_path=save_path)
        print("\n")
    ...
    print("="*70)
    print("\nGenerated files:")
    print("  - pci_bar_chart.png (basic bar plot)")
    print("  - pci_with_error_bars.png (Monte Carlo error bars)")
    print("  - pci_tornado_*.png (tornado plots for each material)")
    print("  - pci_sobol_*.png (Sobol sensitivity plots for each material)")
    print("="*70)
