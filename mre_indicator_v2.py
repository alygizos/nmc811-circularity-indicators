...
def R1_mass(m_waste: float, m_natural_resources: float, m_product_R1: float) -> float:                 # R1 Reduce
    ...
    if m_product_R1 <= 0:
        return float("nan")
    val = (1 - m_waste/m_product_R1) * (m_natural_resources) / m_product_R1
    return val
...
def R4(recycled_fraction: float, qualitative_scores: Sequence[float]) -> float:                         # R4 Recycling
    ...
    if len(Q) == 0:
        return float("nan")
    return  float(recycled_fraction) * (sum(Q)/5/len(Q))
...
def plot_individual_indicators(stats, save_dir='', dpi=300, figsize=(14, 5)):
    """
    ...
    plt.tight_layout()
    save_path_m = os.path.join(save_dir, 'mre_material_indicators.png')
    ...
    plt.tight_layout()
    save_path_r = os.path.join(save_dir, 'mre_recovery_indicators.png')
    ...
    plt.tight_layout()
    save_path_e = os.path.join(save_dir, 'mre_energy_indicators.png')
...
def save_to_cumulative_csv(stats):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    cumulative_csv_path = os.path.join(script_dir, '..', 'cumulative_results_indicators.csv')
...
if __name__ == '__main__':
    ...
    print("="*70)
    plot_individual_indicators(stats)
